...
class MarketingActivity(models.Model):
    ...
    def _compute_inherited_domain(self):
        for activity in self:
            domain = expression.AND([domain_eval(activity.activity_domain or '[]'),
                                     domain_eval(activity.campaign_id.domain or '[]')])
            ancestor = activity.parent_id
            while ancestor:
                domain = expression.AND([domain, domain_eval(ancestor.activity_domain or '[]')])
    ...
    def execute_on_traces(self, traces):
        ...
        if self.domain:
            rec_domain = domain_eval(self.domain)
        else:
            rec_domain = domain_eval(self.campaign_id.domain or '[]')
        if rec_domain:
